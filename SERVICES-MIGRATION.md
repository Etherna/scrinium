...
- **`DateTimeOffset` instead of `DateTime` on model timestamps**: `IEntityModel.CreationDateTime` and the migration operation/log members are `DateTimeOffset` values generated with `UtcNow`; the services' own entity model bases implementing `IEntityModel` must change the property type accordingly. There is no registry-wide default: when a `CreationDateTime` member becomes `DateTimeOffset`, set `DateTimeOffsetSerializer(BsonType.DateTime)` on its member map (sso's configuration on `UserSharedInfo.LockoutEnd` is exactly this pattern and stays as is), otherwise the wire format changes from `ISODate` to the driver default `{DateTime, Ticks, Offset}` document. Stored `ISODate` values keep reading unchanged.
