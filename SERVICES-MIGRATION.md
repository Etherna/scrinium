...
- **A dangling reference now fails loud**: a summary whose referred document doesn't exist anymore on the origin collection denies its full load, throwing `MongodmMissingOriginDocumentException` (before, the model silently gave up its summary state, keeping the members it never loaded at their default values). It surfaces at the first read of a member the summary doesn't carry, at an explicit `LoadValuesAsync`, and also serializing the reference into a document that denormalizes a member missing from the summary — a save of the referencing model. Services deleting referred documents without cleaning the references pointing at them declare `config.MissingOriginDocument = MissingOriginDocumentMode.Warn` (or `Silent`) on those reference serializers, tolerating it per reference.
